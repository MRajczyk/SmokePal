    ...
    if(savingDataFlag) {
        ...
        if(Array.isArray(parsedMessage)) {
            parsedMessage.forEach(async (reading) => {
                try {
                    console.log('creating sensorReading: ', {sessionId: currentSessionId, sensorName: reading.sensorName, value: reading.reading, type: reading.type == 'TEMP' ? ReadingType.TEMP : ReadingType.HUM})
                    await prisma.smokingSensorReading.create({data: {sessionId: currentSessionId, sensorName: reading.sensorName, value: reading.reading, type: reading.type == 'TEMP' ? ReadingType.TEMP : ReadingType.HUM}});
                } catch (e) {
            ...
            try {
                console.log('creating sensorReading: ', {sessionId: currentSessionId, sensorName: parsedMessage.sensorName, value: parsedMessage.reading, type: parsedMessage.type == 'TEMP' ? ReadingType.TEMP : ReadingType.HUM})
                await prisma.smokingSensorReading.create({data: {sessionId: currentSessionId, sensorName: parsedMessage.sensorName, value: parsedMessage.reading, type: parsedMessage.type == 'TEMP' ? ReadingType.TEMP : ReadingType.HUM}});
            } catch (e) {
                console.log(e.message);
                //ignore
            }
...
app.post('/api/start', async (req, res) => {
    ...
    mqttClient.subscribe('esp8266_data', (err) => {
        if (err) {
            return res.status(500).send('An error occured while subscribing to topic \"esp8266_data\"');
        }
    });
    savingDataFlag = true;
    try {
        const newSession = await prisma.smokingSession.create({data: {authorId: req.body.authorId}});
